namespace Neovolve.Toolkit.Storage
{
    ...
    ///   <see cref="IConfigurationStore"/> to create. The configuration key used is <c>ConfigurationStoreType</c> which is defined
                ...
                if (value != null)
                {
                    if (typeof(IConfigurationStore).IsAssignableFrom(value) == false)
                    {
                        throw new InvalidCastException(
                            String.Format(
                                CultureInfo.InvariantCulture, Resources.InvalidCastExceptionMessage, value.GetType(), typeof(IConfigurationStore)));
                    }
                }
